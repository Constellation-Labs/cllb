...
import org.constellation.lb.NemListOps._
...
class NemListOpsSpec extends FunSpec with Matchers {

  describe("extractNewKeys") {
    it("extract new keys") {
      extractNewKeys(NonEmptyMap.of(1 -> List(1, 3, 4), 2 -> List(1, 3, 5)))(identity) shouldBe SortedSet(
        3,
        4,
        5
      )
    ...
    it("no new keys") {
      extractNewKeys(NonEmptyMap.of(1 -> List(1, 3), 2 -> List(2, 1), 3 -> Nil))(identity) shouldBe empty
    }
  }
  ...
  describe("rotateMap") {
    ...
    it("rotate of map of none") {
      rotateMap[Int, Int](NonEmptyMap.of(1 -> Nil))(identity) shouldBe empty
    }

    it("rotate of unit map") {
      rotateMap(NonEmptyMap.of(1 -> List(2 -> "2")))(_._1) shouldBe SortedMap(2 -> List((2 -> "2")))
    }
  ...
  describe("findFirstElement") {
    it("find a session from a nodes map") {
      val nodes = Map(
        Addr(ipv4"127.0.0.1".toInet4Address, 1000) -> List(Info(Id("node-1"), InetAddress.getLoopbackAddress, 9997, 9998, 111L, 123L, NodeState.Ready)),
        Addr(ipv4"127.0.0.30".toInet4Address, 444) -> List(Info(Id("node-2"), InetAddress.getLoopbackAddress, 9999, 10000, 111L, 456L, NodeState.Ready)),
        Addr(ipv4"127.0.0.5".toInet4Address, 88) -> List(Info(Id("node-2"), InetAddress.getLoopbackAddress, 9999, 10000, 111L, 550L, NodeState.Ready))
      )
    ...
    it("find a session from a nodes map with empty values") {
      val nodes = Map(
        Addr(ipv4"127.0.0.30".toInet4Address, 444) -> Nil,
        Addr(ipv4"127.0.0.5".toInet4Address, 88)   -> Nil,
        Addr(ipv4"127.0.0.1".toInet4Address, 1000) -> List(Info(Id("node-1"), InetAddress.getLoopbackAddress, 9997, 9998, 111L, 123L, NodeState.Ready))
      )
      findFirstElement(nodes) shouldBe Some(
        Info(Id("node-1"), InetAddress.getLoopbackAddress, 9997, 9998, 111L, 123L, NodeState.Ready)
