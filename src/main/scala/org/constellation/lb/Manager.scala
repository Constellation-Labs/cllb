...
class Manager(init: NonEmptyList[Addr], config: LoadbalancerConfig)(
  ...
  private lazy val hostsRef =
    Ref.unsafe[IO, NonEmptyMap[Addr, List[Info]]](init.map(addr => addr -> Nil).toNem)
  ...
  def reset: IO[Unit] =
    logger.info("reset") >>
      hostsRef
        .modify(_ => (init.map(addr => addr -> Nil).toNem, ()))
          ...
          buildClusterStatus(status).flatMap {
            case (activeHosts, inactiveHosts) =>
              val clusterHosts = (activeHosts ++ inactiveHosts) // TODO: Add Eviction of inactive hosts
                ...
                .foldLeft(status)((acc, addr) => acc.add(addr, Nil))
  ...
  def buildClusterStatus(init: NonEmptyMap[Addr, List[Info]]): IO[(Set[Addr], Set[Addr])] = {
    ...
    IO {
      val (active, other) = rotateMap(init)(toAddress)
        .partition {
          case (addr, proof) => isActive(addr, proof)
          case _             => false
        }
  ...
  )(implicit client: Client[IO]): IO[NonEmptyMap[Addr, List[Info]]] = {
