...
trait Codecs {
  ...
  implicit val inetCodec: Codec[InetAddress] = Codec.from(
    Decoder.decodeString.emapTry(s => catching(classOf[UnknownHostException]).withTry(InetAddress.getByName(s))),
  ...
  implicit val stateCodec: Codec[NodeState.State] = Codec.from(
    Decoder.decodeString.emapTry(s => catching(classOf[NoSuchElementException]).withTry(NodeState.withName(s))),
